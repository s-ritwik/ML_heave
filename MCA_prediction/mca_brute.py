...
METERS_TO_CM = 25  # keep logs comparable to MCA_GRU
# ------------------------------- Utils --------------------------------------
...
def main():
    ...
    with open(model_log_path, 'a') as mlog:
        ...
        mlog.write(f"Train files: {args.train_files}\n")
        mlog.write(f"Test file: {args.test_file}\n")
        mlog.write(f"Center={args.center}, Energy cutoff={args.energy_cutoff}, P_used={P_used}, Ridge={args.ridge}\n")
        mlog.write(f"Strides: train={args.stride_train}, test={args.stride_test}\n")
        mlog.write(f"Training windows: {M_total}\n")
        mlog.write('='*80 + '\n')
    ...
    # -------------------- Logging (match MCA_GRU format) ----------------------
    abs_diff = np.abs(Y_pred - Y_true)
    avg_abs_error_cm = float(np.mean(abs_diff)) * METERS_TO_CM
    hz = 20
    steps_3s = min(3 * hz, m)
    steps_4s = min(4 * hz, m)
    steps_5s = min(5 * hz, m)
    mae_3s_cm = float(np.mean(abs_diff[:steps_3s, :])) * METERS_TO_CM if steps_3s > 0 else float('nan')
    mae_4s_cm = float(np.mean(abs_diff[:steps_4s, :])) * METERS_TO_CM if steps_4s > 0 else float('nan')
    mae_5s_cm = float(np.mean(abs_diff[:steps_5s, :])) * METERS_TO_CM if steps_5s > 0 else float('nan')
    avg_pred_time_ms = (pred_t1 - pred_t0) * 1000.0 / max(1, K_test)
    ...
    with open(model_log_path, 'a') as mlog:
        mlog.write('-'*80 + '\n')
        mlog.write(f"[{now_str()}] Training finished in {dt:.2f} s\n")
        mlog.write(f"Model directory: {model_dir}\n")
        mlog.write(f"Average Prediction Time: {avg_pred_time_ms:.6f} ms\n")
        mlog.write(f"Average Absolute Error (Total): {avg_abs_error_cm:.6f} cm\n")
        mlog.write(f"Average Absolute Error (3s): {mae_3s_cm:.6f} cm\n")
        mlog.write(f"Average Absolute Error (4s): {mae_4s_cm:.6f} cm\n")
        mlog.write(f"Average Absolute Error (5s): {mae_5s_cm:.6f} cm\n")
        mlog.write('-'*80 + '\n\n')
