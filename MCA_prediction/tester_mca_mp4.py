...
Options:
    ...
    --no-video        Skip MP4 generation; compute metrics only
    --save-data       Optional path to save y_true/y_pred/errors/pred_times as NPZ
"""
...
def main():
    ...
    ap.add_argument("--no-video", action="store_true", help="Skip MP4 generation; still compute metrics.")
    ap.add_argument("--save-data", type=str, default=None, help="If set, save y_true, y_pred, abs_err, pred_times to this NPZ path.")
    args = ap.parse_args()
    ...
    # Determine test file
    test_file = args.test_file or config.get("test_file") or "/MCA_prediction/train_data_normalised_mocap/D1H3_normalised.csv"
    ...
    total_steps = max_i -start_i +1
    print(f"Total steps to predict: {total_steps} (from index {start_i} to {start_i + total_steps - 1})")
    ...
    save_arrays = save_data is not None
    preds_to_save, truths_to_save, abs_err_to_save = [], [], []
    if save_arrays and save_data:
        save_dir = os.path.dirname(save_data)
        if save_dir:
            os.makedirs(save_dir, exist_ok=True)

    # Matplotlib setup (skipped if no-video)
    if not args.no_video:
        ...
        with writer.saving(fig, out_mp4, dpi=300):
            for step in range(total_steps):
                ...
                if save_arrays:
                    preds_to_save.append(yhat.copy())
                    truths_to_save.append(ytrue.copy())
                    abs_err_to_save.append(abs_err.copy())
        ...
        # Metrics-only path (no Matplotlib/FFmpeg)
        for step in range(total_steps):
            i = start_i + step
            x1 = series[i-n+1:i+1].copy()
            if args.noise_std > 0.0:
                x1 = x1 + np.random.normal(0.0, args.noise_std, size=x1.shape)
            ...
            ytrue = series[i+1:i+1+m]
            abs_err = np.abs((yhat - ytrue) * meters_to_cm)
            mean_abs_errors.append(abs_err.mean())
            err_3s.append(abs_err[:steps_3s].mean())
            err_4s.append(abs_err[:steps_4s].mean())
            err_5s.append(abs_err[:steps_5s].mean())
    ...
    if mean_abs_errors:
        ...
        if save_arrays:
            np.savez(
                save_data,
                y_pred=np.array(preds_to_save),
                y_true=np.array(truths_to_save),
                abs_err=np.array(abs_err_to_save),
