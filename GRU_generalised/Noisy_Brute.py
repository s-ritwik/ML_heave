...
def train_and_test_model(config_line):
    ...
    model_name = f"noisy_500epochs_D1Gru_{sequence_length//20}_{output_size//20}_{'_'.join(map(str, hidden_sizes))}.pth"
    ...
    sigma = np.std(X_train) * noise_level

    # Add zero-mean Gaussian noise to X_train
    noise = np.random.normal(0, sigma, X_train.shape)
    X_train_noisy = X_train + noise

    X_train = torch.tensor(X_train_noisy, dtype=torch.float32).unsqueeze(-1)  # Shape: (samples, sequence_length, 1)
    ...
    folder_path = "brute_results_noise"
    ...
    # Add noise to test_data_segment
    noise_level = 0.05  # Use the same noise level as during training
    sigma_test = np.std(test_data_segment) * noise_level
    noise = np.random.normal(0, sigma_test, test_data_segment.shape)
    test_data_segment_noisy = test_data_segment + noise
    ...
    with torch.no_grad():
        real_time_input = test_data_segment_noisy[:sequence_length]
        ...
        for i in range(sequence_length, len(test_data_segment) - output_size):
            ...
            # Calculate errors using the clean test data
            ...
            # Shift the window
            real_time_input = np.append(real_time_input[1:], test_data_segment_noisy[i])
    ...
    video_filename = os.path.join("brute_results", f"{model_name}_noisy_predictions_and_errors.mp4")
