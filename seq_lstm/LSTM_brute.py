        ...
        try:
            ...
            with torch.no_grad(), writer.saving(fig, video_out_path, dpi=VIDEO_DPI):
                for i in range(start_index, end_index):
                    ...
                    errors_3s.append(float(np.mean(abs_error[:steps_3s])) if steps_3s > 0 else 0.0)
                    errors_4s.append(float(np.mean(abs_error[:steps_4s])) if steps_4s > 0 else 0.0)
                    errors_5s.append(float(np.mean(abs_error[:steps_5s])) if steps_5s > 0 else 0.0)
